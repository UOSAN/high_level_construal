...
def clt(participant_id: str, session: str, run_number: str, is_first: bool):
    ...
    thisExp = data.ExperimentHandler(name=expName, version='',
                                     ...
                                     originPath='/Users/pnovak2/src/smoking/high_level_construal/construal_level_task_lastrun.py',
    ...
    _key_resp_allKeys = []
    # keep track of which components have finished
    instructionsComponents = [title_text, instruction, key_resp]
        ...
        for thisScenario_trial in scenario_trials:
            ...
            for thisAction_trial in action_trials:
                ...
                while continueRoutine and routineTimer.getTime() > 0:
                    ...
                    waitOnFlip = False
                    if attention_check_keyboard.status == NOT_STARTED and tThisFlip >= 4 - frameTolerance:
                        # keep track of start time/frame for later
                        ...
                        win.callOnFlip(attention_check_keyboard.clearEvents,
                    ...
                    if attention_check_keyboard.status == STARTED and not waitOnFlip:
                        theseKeys = attention_check_keyboard.getKeys(keyList=['5', '6', '7', '8', '9'],
                                                                     waitRelease=False)
                        _attention_check_keyboard_allKeys.extend(theseKeys)
                        if len(_attention_check_keyboard_allKeys):
                            attention_check_keyboard.keys = _attention_check_keyboard_allKeys[
                ...
                for thisComponent in actionComponents:
                    if hasattr(thisComponent, "setAutoDraw"):
                        thisComponent.setAutoDraw(False)
                attention_check_rating.tStopRefresh = tThisFlipGlobal
                attention_check_keyboard.tStopRefresh = tThisFlipGlobal
            ...
            # update component parameters for each repeat
            scenario_cue_text.setText(
                "Consider the thoughts that would go through your mind if you are in the following situation\n" + thisBlock['cue_text'])
            # keep track of which components have finished
            ...
            for thisComponent in scenario1Components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            block1_text.tStopRefresh = tThisFlipGlobal
            block1_sound.tStopRefresh = tThisFlipGlobal
            ...
            for thisComponent in scenario2Components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            block2_text.tStopRefresh = tThisFlipGlobal
            block2_sound.tStopRefresh = tThisFlipGlobal
            block2_sound.stop()  # ensure sound has stopped at end of routine
            scenario_trials.addData('block2_sound.started', block2_sound.tStartRefresh)
            scenario_trials.addData('block2_sound.stopped', block2_sound.tStopRefresh)
            ...
            for thisComponent in scenario3Components:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            block3_text.tStopRefresh = tThisFlipGlobal
            block3_sound.tStopRefresh = tThisFlipGlobal
            ...
            # update component parameters for each repeat
            quitting_intention_text.setText(
            ...
            while continueRoutine and routineTimer.getTime() > 0:
                ...
                if quitting_intention_keyboard.status == NOT_STARTED and tThisFlip >= 0.0 - frameTolerance:
                    ...
                    win.callOnFlip(quitting_intention_keyboard.clearEvents,
                                   eventType='keyboard')  # clear events on next screen flip
                ...
                if quitting_intention_keyboard.status == STARTED and not waitOnFlip:
                    theseKeys = quitting_intention_keyboard.getKeys(keyList=['5', '6', '7', '8', '9'],
                                                                    waitRelease=False)
                    _quitting_intention_keyboard_allKeys.extend(theseKeys)
                    if len(_quitting_intention_keyboard_allKeys):
                        quitting_intention_keyboard.keys = _quitting_intention_keyboard_allKeys[
                            -1].name  # just the last key pressed
            ...
            for thisComponent in quitting_intentionComponents:
                if hasattr(thisComponent, "setAutoDraw"):
                    thisComponent.setAutoDraw(False)
            quitting_intention_rating.tStopRefresh = tThisFlipGlobal
            quitting_intention_keyboard.tStopRefresh = tThisFlipGlobal
